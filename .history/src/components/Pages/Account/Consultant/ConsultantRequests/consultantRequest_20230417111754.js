...
export const ConsultantRequest = (props) => {
	const [cancelLoaing, setCancelLoaing] = useState(false);
	const [acceptLoaing, setAcceptLoaing] = useState(false);
	const { event, acceptBooking, cancelBooking, auth } = props;

	console.log(auth.uid, event);
	...
	useEffect(() => {}, [cancelload, confirmLoad]);

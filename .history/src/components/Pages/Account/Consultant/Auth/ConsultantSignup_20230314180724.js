...
const ConsultantSignup = (props) => {
	...
	function addService() {
		if (userServiceArray.length < 5) {
			...
			newArray.splice(userServiceArray.length - 1, 0, { name: "", price: "" });
		...
		return (
			<div key={`userService-${index}`}>
				<Row>
					...
					<Col md={1}>
						<button onClick={(e) => deleteService(index)}>D</button>
					</Col>
				</Row>
			</div>
		);
				...
				>
					...
					<Form.Group controlId="formBasicService" className="form-group">
						<Form.Label style={{ width: "100%" }}>
							8.a Service Charge (hourly rate)
							<span style={{ color: "red" }}>*</span>
						</Form.Label>

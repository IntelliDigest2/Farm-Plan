...
function AvailabilityOrganiser(props) {
	...
	const [events, setEvents] = useState(props.consultantCalendarInfo);
	...
	console.log(props.consultantCalendarInfo);
	let localEventArray;
	...
	useEffect(() => {
		if (props.consultantCalendarInfo.length === 0) {
			setEmptyCalendarNotification(true);
		}
	}, [props.consultantCalendarInfo]);
	...
	if (props.consultantCalendarInfo.length > 0) {
		localEventArray = transformUTCtoLocalTime(props.consultantCalendarInfo);
	}
	...
	function addEvent() {
		...
		props.addEventToDB(newEvent, consultantId);
	...
	function setFrequencyOfOccurence(frequency) {
		// clickedDate
	}
					...
					<Row>
						...
						<Col>
							<Form.Group className="mb-3" controlId="availabilityFrequency">
								...
								<Form.Control as="select" onChange={setFrequencyOfOccurence}>

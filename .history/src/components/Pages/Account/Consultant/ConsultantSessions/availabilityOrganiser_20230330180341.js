...
function AvailabilityOrganiser({ consultantCalendarInfo }) {
	...
	function transformUTCtoLocalTime(events) {
		let localTimeEvents = events.map((ev) => {
			...
			var formatLocalDate = function (event) {
				...
				let localMonth = localTime.getMonth();
				let localDay = localTime.getDate();
				let localHour = localTime.getHours();
				let localMinute = localTime.getMinutes();
				return `${localYear}-${localMonth}-${localYear}-T${localDay
					.toString()
					.padStart(2, "0")}:${localHour
					.toString()
					.padStart(2, "0")}:${localMinute.toString().padStart(2, "0")}`;
			};
			...
			return { ...events, start: localStartEvent, end: localEndEvent };
	...
	transformUTCtoLocalTime(consultantCalendarInfo);

	console.log(transformUTCtoLocalTime(consultantCalendarInfo));

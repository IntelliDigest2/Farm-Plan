...
function AvailabilityOrganiser({ consultantCalendarInfo }) {
	...
	function setTime(e) {
		...
		let startDateAndTime = `${clickedDate}T${time}:00`;
	...
	function calculateEndDateAndTime(date, duration) {
		let endDate = new Date(date.getTime() + parseInt(duration) * 60000)
			.toISOString()
			.split(".")[0];
	...
	function setDuration(e) {
		...
		let endDate = calculateEndDateAndTime(date, duration);

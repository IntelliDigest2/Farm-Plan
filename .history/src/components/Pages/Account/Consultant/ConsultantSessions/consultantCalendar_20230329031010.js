...
function ConsultantCalendar({
	...
	useEffect(() => {
		console.log(
			`${new Date(calendarRef.current.getApi().getDate()).getDay}-${
				new Date(calendarRef.current.getApi().getDate()).getMonth
			}`
		);
		passCurrentDate(calendarRef.current.getApi().getDate());

	...
	function transformUTCtoLocalTime(events) {
		let localTimeEvents = events.map((ev) => {
			...
			var formatLocalDate = function (event) {
				...
				let localMonth = localTime.getMonth().padStart(2, "0");
				let localDay = localTime.getDate().padStart(2, "0");
				let localHour = localTime.getHours().padStart(2, "0");
				let localMinute = localTime.getMinutes().padStart(2, "0");
				return `${localYear}-${localMonth}-${localYear}-T${localDay}:${localHour}:${localMinute}`;
			};

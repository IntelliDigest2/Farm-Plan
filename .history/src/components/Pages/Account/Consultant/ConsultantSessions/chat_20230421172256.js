...
function Chat(props) {
	...
	const sendMessage = async (e) => {
		...
		try {
			...
			console.log(sentMessage.data, "this show the sent msg has come back");

			socket.emit("new_message", sentMessage.data);
		} catch (err) {
			console.log(err);
		}
		// }
	...
	if (messages.length > 0) {
		allMessages = messages.map(({ content, senderId }) => {
			return (
				<div
	...
	const typingHandler = (e) => {
		...
		setNewMessage({ content: e.target.value, senderId: fakeuid });

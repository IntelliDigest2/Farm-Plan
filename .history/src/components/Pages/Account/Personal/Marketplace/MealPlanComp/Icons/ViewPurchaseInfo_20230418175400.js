				...
				<>
					<List>
						{props.bookingsInfo.map(({ bookingId, booking }, index) => {
							...
							let bookingEvent = Object.keys(booking.event);
							...
							return (
								...
								>
									<Table striped bordered hover>
										...
										{/* <tbody>
											{bookingEvent.map((ev) => {
												let date = format(parseISO(ev), "yyyy-mm-dd");
												let startTime = format(parseISO(ev.start), "hh:mm a");
												let endTime = format(parseISO(ev.end), "hh:mm a");

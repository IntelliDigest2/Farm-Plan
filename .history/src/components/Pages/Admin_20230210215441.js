...
const Admin = () => {
	...
	let accordions = accordionInfos.map((accordionInfo, i) => {
		return (
			<Accordion
				userName={accordionInfo.userName} // useRef={(el) => (accordionRef.current[i] = el)}
				location={accordionInfo.location} // useRef={(el) => (accordionRef.current[i] = el)}
				status={accordionInfo.status} // useRef={(el) => (accordionRef.current[i] = el)}
				products={accordionInfo.products} // useRef={(el) => (accordionRef.current[i] = el)}

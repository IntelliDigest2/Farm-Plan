...
function BookConsulting(props) {
	...
	if (result) {
		displayResult = result.map((consultant, index) => {
			...
			let displayDayEvents = dayEvents.map((e) => {
				console.log(e);
				return (
					<div>
						...
						<div>Event Start: {e.start.split("T")[1]}</div>
						<div> Event end: {e.end.split("T")[1]}</div>
						<button onClick={bookConsultantEvent}></button>
					</div>
				);
			});

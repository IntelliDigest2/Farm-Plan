...
const Accordion = ({ userName, location, products, status, date }) => {
	...
	let productsInfo = products.map((product) => {
		return (
			...
			>
				...
				<ProductRequestInfo key={`extraInfo-${uuidv4()}`} ref={extraInfoRef} />
			</div>
		);
	});
	...
	function extraInfoHandler(value) {
		console.log(value);
		if (value) {
			extraInfoRef.current.style.visibility = "visible";
			console.log(extraInfoRef.current.backgroundColor);
		} else {
			extraInfoRef.current.style.visibility = "hidden";
		}
	}

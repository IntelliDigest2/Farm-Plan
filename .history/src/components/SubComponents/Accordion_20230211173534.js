...
const Accordion = ({ userName, location, products, status, date }) => {
			...
			<div>
				...
				<div className="accordion_productUpdate_input">
					<input
						...
						value={productPricingForm.values[`${product.name}-qty${i}`]}
					></input>
		...
		return (
			<tr>
				...
				<td>
					<input
						id={`${product.name}`}
						name={`${product.name}-sply${i}`}
						type="number"
						placeholder="0"
						onChange={productPricingForm.handleChange}
						value={productPricingForm.values[`${product.name}-sply${i}`]}
					></input>
				</td>
				<td>
					<select name={`${product.name}-unit${i}`} id="unit-select">
						<option value="kg">kg</option>
						<option value="g">g</option>
						<option value="ltr">ltr</option>
						<option value="unit">unit</option>
	...
	let productsData = products.map((product, i) => {
		return <Row product={product} i={i}></Row>;
	});

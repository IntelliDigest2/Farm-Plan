...
function BookConsulting(props) {
	...
	if (result) {
		displayResult = result.map((consultant, index) => {
				...
				let localTimeEvents = events.map((ev) => {
					var formatLocalDate = function (event) {
						...
						return `${localYear}-${localMonth
							.toString()
							.padStart(2, "0")}-${localDay
							.toString()
							.padStart(2, "0")}T${localHour
							.toString()
			...
			let formatedEvents = transformUTCtoLocalTime(consultant.events);
				...
				<div key={index}>
					...
					{showEventsForSelectedDay ? (
						<div>
							...
							<div>
								<div>
									{formatedEvents.find((event, clickedDay) => {
										console.log(event.start.split("T")[0] === clickedDay;)

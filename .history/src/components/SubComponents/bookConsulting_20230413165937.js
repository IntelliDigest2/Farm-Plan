...
function BookConsulting(props) {
	...
	const { consultingResult, getConsultants, isFetchingData, bookEvent } = props;
	...
	const bookConsultantEvent = (e, event, consultantId) => {
		...
		bookEvent(eventToSend, consultantId);
	};
	...
	if (result) {
		displayResult = result.map(({ consultant, consultantId }, index) => {
			...
			let displayDayEvents = dayEvents.map((event) => {
				// console.log(event);
				return (
					<div>
						...
						<Button
							...
							disabled={event.status === "requested" ? true : false}
						>
							{event.status === "requested" ? "Requested" : "Book Opening"}
						</Button>
					</div>
				);
			});
			...
			return (
				<div key={index}>
					...
					) : !displayDayEvents.length ? (
						`click day on calendar to see events`
					) : (
						`no event for ${clickedDay}`
					)}
				</div>
			);
		});
...
const mapDispatchToProps = (dispatch) => {
	return {
		...
		bookEvent: (event, consultantId) => {
			dispatch(bookEvent(event, consultantId));
		},
	};
};

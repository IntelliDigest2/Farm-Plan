...
function createChat(uid, userName, consultantId, consultantName) {
	console.log(uid, userName, consultantId, consultantName,`these are the information we want to send to the chat);
	try {
	...
	return (dispatch, getState, { getFirestore }) => {
		...
		getFirestore()
			...
			.then((result) => {
				...
				if (eventType === "Chat") {
					createChat(uid, profile.userName, consultantId, consultantName);
				}
			})
			.catch((err) => {
				console.log(err);

...
export const addConsultantEventToDatabase = (newEvent, consultantId) => {
	return (dispatch, getState, { getFirebase, getFirestore }) => {
		console.log(newEvent, consultantId);
		console.log(Object.getOwnPropertyNames(getFirebase()));
		getFirestore()
			.collection("consultants")
			.doc(consultantId)
			.update({
...
export const sendChatMessageToApi = (newMessage, chatId) => {
	return (dispatch) => {
		axios
			.post(`/api/message`, {
				content: newMessage,
			...
			.then((data) => {
				dispatch({ type: "SEND_MESSAGES_SUCCESS", payload: data });
			})
			.catch((err) => {
				console.log(err);

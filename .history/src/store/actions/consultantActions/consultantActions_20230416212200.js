...
export const acceptRequest = (consultantId, event) => {
	return (dispatch, getState, { getFirebase, getFirestore }) => {
		dispatch({
...
export const cancelRequest = (event, consultantId) => {
	return (dispatch, getState, { getFirebase, getFirestore }) => {
		dispatch({
			type: "CONSULTATION_REQUEST_LOAD",
		});
		...
		consultantRef
			...
			.then(() => {
				dispatch({
					type: "CONSULTATION_REQUEST_SUCCESS",
				});
			})
			.catch((err) => {
				dispatch({
					type: "CONSULTATION_REQUEST_FAILED",
				});
			});
	};
};

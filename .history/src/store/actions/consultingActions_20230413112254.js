		...
		getFirestore()
			...
			.onSnapshot(
				(doc) => {
					...
					let consultants = [];
					doc.forEach((doc) => {
						consultants.push({ consultant: doc.data(), id: doc.id });
...
export const bookEvent = (consultantId, event) => {
	return (dispatch, getState, { getFirebase, getFirestore }) => {
		getFirestore()
			.collection("consultants")
			.doc(consultantId)
			.update({
				// calendarEvents: firebase.firestore.FieldValue.arrayUnion(eventUpdate),
			})
			.update();
	};
};

...
import {Form, Button, Card, Col, Row, InputGroup, DropdownButton, Dropdown} from 'react-bootstrap';
import { connect } from 'react-redux';
import { startData, createFoodSurplusData } from '../../../store/actions/dataActions';
...
class AddProductsBusiness extends Component {
    ...
    state = {
        ...
        submissionType: "Surplus Business",
        ...
        testProductlist: [
            {foodName: "Chicken", category: "Meat", weight: 240, wtUnit: "g", price: 2.99, expiryDate: "2021-10-07", postcode: "ML6 9TY"},
            {foodName: "Milk", category: "Dairy", weight: 1, wtUnit: "l", price: 0.99, expiryDate: "2021-09-30", postcode: "ML5 4QX"}, 
    ...
    componentDidMount() {
        ...
        if (isMobile){
            this.setState({formWidth: "90vw", formHeight: "885px", dropdownWidth: "241px"})
        }
        else if (isBrowser){
            this.setState({formWidth: "783px", formHeight: "885px", dropdownWidth: "610px"})
        }
    }
    ...
    render() {
        ...
        return(
            ...
            >
            <MobileView><h6 style={{paddingTop: '8vh', color: 'black', justifyContent: 'center', display: 'flex'}}>Upload Food (Business)</h6></MobileView>
            <BrowserView><h4 style={{paddingTop: '8vh', color: 'black', justifyContent: 'center', display: 'flex'}}>Upload Food (Business)</h4></BrowserView>
        ...
        <Col className="mt-5 pt-5" xs={12}></Col>
          <Col className="" xs={12} lg={4}></Col>
            ...
            <Form className= "form-layout" style={{padding: "10px"}}>  
                <h5 className="text-center" style={{margin: "30px", fontSize: "23px",fontWeight: "600"}}>Add Products</h5>
                ...
                <div>
                    ...
                    <Form.Group
                        ...
                        <InputGroup>
                            <DDMenuStyle>
                                <Dropdown>
                                    <DropdownToggle variant="secondary" style={{width: this.state.dropdownWidth}} className="dd">{this.state.foodCategory}</DropdownToggle>
                                    <DropdownMenu>
                    ...
                    <div style={{padding: "0 10% 0 10%"}}>Postcode</div>
                        ...
                        <Divider style={{marginBottom: "10px"}}/>
                            ...
                            <>{this.state.showComposition ?
                                <div onClick={() => this.setState({showComposition: !this.state.showComposition, formHeight: "885px"})} style={{padding: "0 10% 0 10%", fontWeight: "bold"}}>Food Composition  <BsFillCaretUpFill /> </div>
                                :
                                <div onClick={() => this.setState({showComposition: !this.state.showComposition, formHeight: "1245px"})} style={{padding: "0 10% 0 10%", fontWeight: "bold"}}>Food Composition  <BsFillCaretDownFill /> </div>
                            }</>
                        ...
                        <>{this.state.showComposition ? <Divider style={{marginBottom: "10px"}}/> : <Divider style={{marginBottom: "10px", marginTop: "10px"}}/>}</>
                        ...
                        >
                        <InputGroup>
                            <Form.Control  type="number" id="foodSurplusWeight" placeholder="Enter weight of food waste" onChange={(e) => {this.handleFoodWasteGHGChange(e); this.handleFoodCostChange(e)}} width="100%" value={this.state.foodSurplusWeight}/>
                        ...
                        <InputGroup>
                            <DDMenuStyle>
                                <Dropdown>
                                    <DropdownToggle variant="secondary" style={{width: this.state.dropdownWidth}} className="dd">{this.state.producedLocally}</DropdownToggle>
                                    <DropdownMenu>
                                        <DropdownItem as="button" type="button">
                                            <div onClick={(e) => this.handleProducedLocallyChange(e.target.textContent)}>
                                                Local Produce
                        ...
                        >
                        <InputGroup>
                            <Form.Control type="number" id="GHG" placeholder="Enter GHG value" value={(this.state.ghg*this.state.weightMultiplier).toFixed(2)} width="100%"/>{/*<p style={{width:'100px'}}>kg co2</p>*/}
                            <InputGroup.Append>
                                <InputGroup.Text style={{backgroundColor: "#040335", color: "white"}}>kg co2</InputGroup.Text>
                            </InputGroup.Append>
                        </InputGroup>
                        </Form.Group>
                        ...
                        <InputGroup>
                            ...
                            <DropdownButton
                                ...
                                variant="outline-secondary"
                                title={<span style={{color: "white"}}>{this.state.currency}</span>}
                            ...
                            >
                                ...
                                {/* as="button" */}
                                <DropdownItem as="button" type="button">
                                    <div onClick={(e) => {this.changeCurrency(e.target.textContent); this.changeCurrencyMultiplier(1)}}>
                                        GBP (£)
                                    </div>
                                </DropdownItem>
                                ...
                                {/* as="button" */}
                                <DropdownItem as="button" type="button">
                                    <div onClick={(e) => {this.changeCurrency(e.target.textContent); this.changeCurrencyMultiplier(1.404)}}>
                                        USD ($)
                                    </div>
                                </DropdownItem>
                                ...
                                {/* as="button" */}
                                <DropdownItem as="button" type="button">
                                    <div onClick={(e) => {this.changeCurrency(e.target.textContent); this.changeCurrencyMultiplier(1.161)}}>
                                        EUR (€)
                                    </div>    
                                </DropdownItem>
                        ...
                        <Form.Control type="number" id="foodSurplusCost" placeholder="Enter cost of food surplus" value={(this.state.foodSurplusCost*this.state.currencyMultiplier*this.state.weightMultiplier).toFixed(2)}/>{/*pounds*/}
                        </InputGroup>
                        </Form.Group>
                        ...
                        <div>{ this.state.foodName !== "" && this.state.foodCategory !== "Select Category" && this.state.foodSurplusWeight !== 0 && this.state.weightType !== "Select Unit" && this.state.postcode !== "" && this.state.producedLocally !== "Select" && this.state.price !== 0.00 && this.state.expiryDate !== "" && this.state.currency !== "Select Currency" ?
                            <Button style={{margin: "0 10% 0 10%", backgroundColor: "#040335", width: "80%", marginTop: "5px"}} onClick={(e) => { this.handleFoodSurplusSubmit(e); this.notificationTest(); this.clearEFWForm() }} variant="secondary" type="button">
                                Update
                            </Button>

                            :

                            <Button style={{margin: "0 10% 0 10%", width: "80%", marginTop: "5px", backgroundColor: "gray"}} disabled>
                                Update
                            </Button>
                        }</div>
            ...
            >
                ...
                <h5 className="text-center" style={{fontWeight: 600, color: "white", marginTop: "2.5%", marginBottom: "5%"}}>My Products</h5>

                <div>
                    {this.state.myProducts.map(product => (
                        <>
                            <BrowserView>
                                <Card style={{width: "95%", marginLeft: "2.5%", backgroundColor: "rgb(38, 120, 214)", height: "100px", marginBottom: "50px"}}>
                                    <ExpDate className="exp-date" style={{width: "15%", marginLeft: "2.5%", height: "92.5%", marginTop: "0.75%"}}>
                                        <label>EXPIRY DATE</label>
                                        <div className='exp-date__month'>{this.monthConverter(product.expiryDate.split("-")[1])}</div>
                                        <div className='exp-date__day'>{product.expiryDate.split("-")[2]}</div>
                                        <div className='exp-date__year'>{product.expiryDate.split("-")[0]}</div>
                                    </ExpDate>

                                    <div style={{marginLeft: "20%", marginTop: "-12%"}}>
                                        <li><b>Name:</b> {" "} {product.foodName} </li>
                                        <li><b>Category:</b> {" "} {product.category} </li>
                                        <li><b>Postcode:</b> {" "} {product.postcode} </li>
                                        <li><b>Weight/Amount:</b> {" "} {product.weight}{product.wtUnit} </li>
                                    </div>

                                    <div>
                                        <div className="exp-date" style={{marginLeft: "80%", height: "20px", width: "15%", marginTop: "-11%"}}>
                                            Price: £{product.price}
                                        </div>
                            ...
                            <MobileView>
                                <Card style={{width: "95%", marginLeft: "2.5%", backgroundColor: "rgb(38, 120, 214)", height: "100px", marginBottom: "50px"}}>
                                    <ExpDate className="exp-date" style={{width: "25%", marginLeft: "2.5%", height: "95%", marginTop: "1%"}}>
                                    ...
                                    <div style={{marginLeft: "30%", marginTop: "-26.5%"}}>
                                        <li style={{fontSize: "85%"}}><b style={{fontSize: "85%"}}>Name:</b> {" "} {product.foodName} </li>
                                        <li style={{fontSize: "85%"}}><b style={{fontSize: "85%"}}>Category:</b> {" "} {product.category} </li>
                                        <li style={{fontSize: "85%"}}><b style={{fontSize: "85%"}}>Postcode:</b> {" "} {product.postcode} </li>
                                        <li style={{fontSize: "85%"}}><b style={{fontSize: "85%"}}>Weight/Amount:</b> {" "} {product.weight}{product.wtUnit} </li>
                                    </div>

                                    <div>
                                        <div className="exp-date" style={{marginLeft: "72.5%", height: "60px", width: "25%", marginTop: "-22.5%", fontSize: "85%", marginBottom: "1.5%", fontSize: "90%"}}>
                                            Price: £{product.price}
                                        </div>
...
export default compose(connect(mapStateToProps, mapDispatchToProps),firestoreConnect([{ collection: "data" }]))(AddProductsBusiness);
// export default AddProductsBusiness
...
const FoodWaste = (props) => {
  ...
  //Update foodWasteCost when foodWasteWeight or weightType changes
  useEffect(() => {
    setUpload({
      ...upload,
      foodWasteCost: (Number(upload.foodWasteWeight) * 0.85).toFixed(2),
    });
  }, [upload.foodWasteWeight, upload.weightType]);
        ...
        <FormGroup className="mb-3">
          ...
          <InputGroup>
            <Form.Control
              id="foodWasteCost"
              value={
                props.upload.foodWasteCost *
                props.multipliers.currencyMultiplier *
                props.multipliers.weightMultiplier
              }
              readOnly
            />
            <Dropdown

...
const CreateSchoolCode = (props) => {
  ...
  const sendEmail = async (email, name, schoolCode) => {
    try {
      ...
      const response = await fetch(`${baseUrlProd}/v1/auth/send-school-code`, {
        ...
        body: JSON.stringify({ email: email, name: name, schoolCode: schoolCode}),
      });
  ...
  const genrateSchoolCode = async ({ email, name }) => {
    try {
      ...
      const generate = await fetch(`${baseUrlProd}/v1/auth/create-school-code`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
...
const handleAddEmail = () => {
 ...
 const handleAddName = () => {
  ...
  // Filter out empty strings and duplicate emails
  const uniqueNames = Array.from(new Set(newNames.filter(name => name !== '')));

  // Filter out emails already in the list
  const filteredUniqueNames = uniqueNames.filter(names => !namesList.includes(names));
...
const handleRemoveEmail = (email) => {
  ...
  if (indexToRemove !== -1) {
    ...
    const newEmailList = [...emailList.slice(0, indexToRemove), ...emailList.slice(indexToRemove + 1)];
    const newNameList = [...namesList.slice(0, indexToRemove), ...namesList.slice(indexToRemove + 1)];
  ...
  if (file) {
    ...
    reader.onload = (e) => {
      try {
        ...
        const emails = rows.map(row => row[emailColumnIndex]).filter(email => isValidEmail(email));
 ...
 const handleCloseModal = () => {
  ...
  return (
      <div className="page-container">
        ...
        <Form>
            <div className="row">
              <div className="col-md-6"> {/* Adjust the column width as needed */}
              ...
              <div className="col-md-6"> {/* Adjust the column width as needed */}
                <Form.Group controlId="fullName">
                  ...
                  <div className="ml-2"> {/* Add margin between the input and button */}
                    <Button 
                      onClick={() => {
                        handleAddEmail()
                        handleAddName()
          ...
          <div className="d-flex align-items-center justify-content-between mt-3">
            <Button variant="primary" onClick={handleSendEmails} disabled={isLoading}>
              {isLoading ? 'Creating...' : 'Create School Code'}
            </Button>

            <div className="ml-3">
              {emailList.length > 0 && (
                <Button variant="danger" onClick={handleRemoveAllEmails} className="mr-2">
                  Remove All
                </Button>
              )}
            </div>
